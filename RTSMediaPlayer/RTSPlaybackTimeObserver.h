 ...
 *  therefore especially useful when regularly retrieving metadata from a webservice during playback, for example.
 *
 *  Since a playback time observer calls the associated block only when the associated player is actually playing,
 *  stream information (e.g. duration, current time) is available and reliable within the block implementation. A
