...
- (BOOL) isLive
{
	...
	return self.playerController.streamType == RTSMediaStreamTypeLive
		|| (self.playerController.streamType == RTSMediaStreamTypeDVR && (self.maximumValue - self.value < RTSMediaLiveTolerance));
}

- (void) updateTimeRangeLabels
{
	AVPlayerItem *playerItem = self.playerController.playerItem;
...
- (BOOL) continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	// is valuable
	if (self.playerController.mediaType == RTSMediaTypeVideo) {
...
{
	if ([self isDraggable]) {
		[self.playerController playAtTime:self.time];
	}
...
- (void)drawDownloadProgressValueBar:(CGContextRef)context
{
	...
	for (NSValue *value in self.playerController.playerItem.loadedTimeRanges) {
...
- (void)drawTimeRangeProgress:(CMTimeRange)timeRange context:(CGContextRef)context
{
	...
	CGFloat duration = CMTimeGetSeconds(self.playerController.playerItem.duration);
	if (isnan(duration))
		return;
...
- (void)willMoveToWindow:(UIWindow *)window
{
	...
	if (window) {
		@weakify(self)
		self.periodicTimeObserver = [self.playerController addPeriodicTimeObserverForInterval:CMTimeMake(1., 5.) queue:NULL usingBlock:^(CMTime time) {
			@strongify(self)
			
			if (!self.isTracking && self.playerController.playbackState != RTSMediaPlaybackStateSeeking)
			{
				CMTimeRange timeRange = [self.playerController timeRange];
				...
				RTSMediaPlayerLogTrace(@"Range min = %@ (value = %@) --- Current = %@ (value = %@) --- Range max = %@ (value = %@)",
									   ...
									   @(CMTimeGetSeconds(self.playerController.playerItem.currentTime)), @(self.value),
		...
		[self.playerController removePeriodicTimeObserver:self.periodicTimeObserver];
	}
}
