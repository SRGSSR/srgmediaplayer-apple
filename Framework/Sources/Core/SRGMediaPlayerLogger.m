...
@implementation SRGMediaPlayerLogger
...
{
    char *logLevelString = getenv("SRGMediaPlayerLogLevel");
    ...
    NSLog(@"[SRGMediaPlayer] %@", [[NSString alloc] initWithFormat:format arguments:arguments]);
    va_end(arguments);
}

@end

Class SRGMediaPlayerLogClass(void)
{
    ...
    dispatch_once(&once, ^{
        ...
        if (! [logClass methodSignatureForSelector:@selector(log:level:flag:context:file:function:line:tag:format:)]) {
            logClass = [SRGMediaPlayerLogger class];
        }
    });
    return logClass;
}
